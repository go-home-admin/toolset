...
func getObjectStrFromRef(ref string, swagger openapi.Spec) string {
	...
	if _, ok := swagger.Definitions[ref]; ok {
		for key, schema := range swagger.Definitions[ref].Properties {
			...
			params = append(params, fmt.Sprintf(`%v:%v`, key, getJsType(ref, schema, swagger)))
		}
	}
...
func getJsType(preRef string, schema *openapi.Schema, swagger openapi.Spec) string {
	t := schema.Type
	switch schema.Type {
	case "integer", "Number":
		t = "number"
	case "array":
		t = "[]"
		if schema.Items != nil {
			if preRef == strings.Replace(schema.Items.Ref, "#/definitions/", "", 1) {
				t = "{}" + t
			} else {
